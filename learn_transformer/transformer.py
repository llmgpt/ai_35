...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        ...
        self.register_buffer('pe', pe) # 不可训练，保存为模型

    def forward(self, x):
        """
        x: [seq_len, batch_size, d_model]
        """
        x = x + self.pe[:x.size(0), :] # 3.4 截取到同样长度的句子长度，其它不变
...
# 3.2.查看编码器结构，需要源词库长度src_vocab_size
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.layers = nn.ModuleList([EncoderLayer() for _ in range(n_layers)]) # 重复6个

    def forward(self, enc_inputs):
        """
        enc_inputs: [batch_size, src_len]，句子长度
        """
...
# 3.1从总体到局部，先看前向，再看对应结构,需要词嵌入维度d_model和目标词库长度tgt_vocab_size
